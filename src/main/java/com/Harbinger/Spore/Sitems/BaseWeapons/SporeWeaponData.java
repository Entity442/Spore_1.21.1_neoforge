...
public interface SporeWeaponData {
    ...
    default void setAdditionalDamage(double value, ItemStack stack) {
        stack.set(SdataComponents.ADDITIONAL_DAMAGE.get(), (float) value);
    }

    default double getAdditionalDamage(ItemStack itemStack) {
        return itemStack.getOrDefault(SdataComponents.ADDITIONAL_DAMAGE.get(), 0.0f);
    }
    ...
    default int getAdditionalDurability(ItemStack stack) {
        return stack.getOrDefault(SdataComponents.WEAPON_EXTRA_DURABILITY.get(), 0);
    }

    default void setAdditionalDurability(int value, ItemStack stack) {
        stack.set(SdataComponents.WEAPON_EXTRA_DURABILITY.get(), value);
    }
    ...
    default int getTypeVariant(ItemStack stack) {
        return stack.getOrDefault(SdataComponents.WEAPON_MUTATION.get(), 0);
    }

    default void setVariant(SporeToolsMutations variant, ItemStack stack) {
        stack.set(SdataComponents.WEAPON_MUTATION.get(), variant.getId() & 255);
    }
    ...
    default void doEntityHurtAfterEffects(ItemStack stack, LivingEntity victim, LivingEntity entity) {
        ...
        if (getVariant(stack) == SporeToolsMutations.BEZERK && Math.random() < 0.3) {
            if (Math.random() < 0.5) {
                entity.addEffect(new MobEffectInstance(Utilities.wrapHolder(MobEffects.MOVEMENT_SPEED.value()), 60, 0));
            } else if (Math.random() < 0.5) {
                entity.addEffect(new MobEffectInstance(Utilities.wrapHolder(MobEffects.DAMAGE_BOOST.value()), 60, 0));
            } else {
                entity.addEffect(new MobEffectInstance(Utilities.wrapHolder(MobEffects.SATURATION.value()), 60, 0));
            }
        }
    }
