...
public class InfectedPickaxe extends SporePickaxeItems {
    ...
    private List<StoreDouble> getBlockChange(){
        ...
        for (String str : SConfig.DATAGEN.block_infection.get()){
            ...
            Block blockCon1 = BuiltInRegistries.BLOCK.get(ResourceLocation.parse(string[0]));
            Block blockCon2 = BuiltInRegistries.BLOCK.get(ResourceLocation.parse(string[1]));
        ...
        if (!values.isEmpty()) {
            ...
            for (StoreDouble storeDouble : values){
                if (storeDouble.value1.equals(state.getBlock()) && !storeDouble.value1.defaultBlockState().isAir()){
                    ...
                    level.addParticle(ParticleTypes.ANGRY_VILLAGER,pos.getX(),pos.getY(),pos.getZ(),0,0.2f,0);
                    //Fix later
                }
            }
        }
    ...
    public InteractionResult useOn(UseOnContext context) {
        ...
        if (player instanceof ServerPlayer serverPlayer && tooHurt(context.getItemInHand())) {
            serverPlayer.playNotifySound(Ssounds.INFECTED_PICKAXE.value(), SoundSource.AMBIENT,1F,1F);
    ...
    private void lookForOres(BlockPos start, Level level, ServerPlayer player){
        ...
        for (BlockPos pos : BlockPos.betweenClosed(
            ...
            if (level.getBlockState(pos).is(ORE_TAG)){
