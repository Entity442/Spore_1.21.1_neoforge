...
public class SlasherModel<T extends Slasher> extends EntityModel<T> {
	...
	public static final ModelLayerLocation LAYER_LOCATION = new ModelLayerLocation( ResourceLocation.fromNamespaceAndPath(Spore.MODID, "slashermodel"), "main");
	...
	public void setupAnim(T entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
		...
		if (entity.getAttackAnimationTick() > 0) {
			...
			if (entity.getVariant() == SlasherVariants.PIERCER){
				 f1 = -1.0F + 0.5F * Mth.abs(20 -  entity.getAttackAnimationTick()) / 6.5F;
			}
	...
	public void renderToBuffer(PoseStack poseStack, VertexConsumer vertexConsumer, int packedLight, int packedOverlay, int alpha) {
		...
		HeadJoint.render(poseStack, vertexConsumer, packedLight, packedOverlay, alpha);
		Marm.render(poseStack, vertexConsumer, packedLight, packedOverlay, alpha);
	}
}