...
public class BusserModel<T extends Busser> extends EntityModel<T> {
	...
	public static final ModelLayerLocation LAYER_LOCATION = new ModelLayerLocation(ResourceLocation.fromNamespaceAndPath(Spore.MODID, "bussermodel"), "main");
	...
	private void setupHeadAnimations(float headPitch, float ageInTicks) {
		...
		jaw.xRot = Mth.sin(ageInTicks * 0.125f) * 0.1f; // ageInTicks/8 divided by 10
	}

	private void setupTailClawAnimations(float ageInTicks) {
		tailClaws.xRot = Mth.sin(ageInTicks * 0.25f) * 0.333f; // ageInTicks/4 divided by 3
		tailClaws2.xRot = Mth.sin(ageInTicks * 0.25f) * 0.5f;  // ageInTicks/4 divided by 2
	}

	private void handleSwimmingAnimations(T entity, float limbSwing, float limbSwingAmount, float ageInTicks) {
		setupSwimmingTailAnimations(limbSwing, limbSwingAmount);
		setupSwimmingArmAnimations(ageInTicks);
	}
	...
	private void setupSwimmingArmAnimations(float ageInTicks) {
		float armWave = Mth.sin(ageInTicks * 0.2f) * 0.166f; // ageInTicks/5 divided by 6
		float forearmWave = Mth.sin(ageInTicks * 0.333f) * 0.142f; // ageInTicks/3 divided by 7

		rightArm.zRot = armWave;
		leftArm.zRot = -armWave;

		// Only apply forearm rotation during specific parts of the wave
	...
	private void handleLandAnimations(T entity, float limbSwing, float limbSwingAmount, float ageInTicks) {
		resetTailYRotations();
		setupLandArmAnimations(ageInTicks);
		setupLandTailAnimations(entity, ageInTicks);
	}
	...
	private void setupLandTailAnimations(T entity, float ageInTicks) {
		...
		if (entity.isVehicle()) {
			...
			// Normal land animation - entire tail sways
			tail.xRot = TAIL_BASE_ROTATION + tailSway;
			tail2.xRot = TAIL_BASE_ROTATION + tailSway;
			tail3.xRot = TAIL_BASE_ROTATION + tailSway;
        }
        tail4.xRot = TAIL_BASE_ROTATION + tailSway;
    }
