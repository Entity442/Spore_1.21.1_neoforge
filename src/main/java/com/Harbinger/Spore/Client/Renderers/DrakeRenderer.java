...
public class DrakeRenderer<Type extends Verfalldrachen> extends CalamityRenderer<Type , EntityModel<Type>> {
    ...
    private static final ResourceLocation NO_WINGS = ResourceLocation.fromNamespaceAndPath(Spore.MODID,
            "textures/entity/dragon/verfalldrache_wings_down.png");
    private static final ResourceLocation NO_RIGHT_WING = ResourceLocation.fromNamespaceAndPath(Spore.MODID,
            "textures/entity/dragon/verfalldrache_right_wing.png");
    private static final ResourceLocation NO_LEFT_WING = ResourceLocation.fromNamespaceAndPath(Spore.MODID,
    ...
    private static final ResourceLocation ELECTRICAL_HEAD = ResourceLocation.fromNamespaceAndPath(Spore.MODID,
            "textures/entity/dragon/verfa_elec_head.png");
    private static final ResourceLocation ELECTRICAL_NECK = ResourceLocation.fromNamespaceAndPath(Spore.MODID,
            "textures/entity/dragon/verfa_elec_seg_start.png");
    private static final ResourceLocation SONIC_HEAD = ResourceLocation.fromNamespaceAndPath(Spore.MODID,
            "textures/entity/dragon/sonic_dragon_head.png");
    private static final ResourceLocation SONIC_NECK = ResourceLocation.fromNamespaceAndPath(Spore.MODID,
            "textures/entity/dragon/verfa_sonic_seg.png");
    private static final ResourceLocation TAR_HEAD = ResourceLocation.fromNamespaceAndPath(Spore.MODID,
            "textures/entity/dragon/tar_dragon_head.png");
    private static final ResourceLocation TAR_NECK = ResourceLocation.fromNamespaceAndPath(Spore.MODID,
            "textures/entity/dragon/verfa_tar_seg.png");

    private static final ResourceLocation TAR_EYES = ResourceLocation.fromNamespaceAndPath(Spore.MODID,
            "textures/entity/dragon/eyes/tar_dragon_eyes.png");
    private static final ResourceLocation SONIC_EYES = ResourceLocation.fromNamespaceAndPath(Spore.MODID,
            "textures/entity/dragon/eyes/sonic_dragon_eyes.png");
    private static final ResourceLocation ELEC_EYES = ResourceLocation.fromNamespaceAndPath(Spore.MODID,
    ...
    private void renderTentacle(PoseStack stack, Type type, int light, MultiBufferSource buffer, Vec3[] segments,int segment, LivingEntity parent, float partial,LIMB limb) {
        ...
        for (int i = 0; i < segments.length; i++) {
            ...
            renderConnection(origin, currentPos,type,light, stack, buffer, i,partial,color,i == 1,i == segments.length-1,limb);
            origin = currentPos;
        }
    }
    private void renderConnection(Vec3 from, Vec3 to,Type parent,int light, PoseStack stack, MultiBufferSource buffer,int index, float partial
            ,int color,boolean first,boolean last,LIMB limb) {
    ...
    private void renderEyes(EntityModel<Type> head,ResourceLocation location,MultiBufferSource buffer,PoseStack stack){
        ...
        head.renderToBuffer(stack,eyes,15728640, OverlayTexture.NO_OVERLAY, -1);
    }
    private void renderCharge(EntityModel<Type> brain, Type livingEntity,MultiBufferSource buffer,PoseStack stack, int packedLight,float partialTicks){
        if (brain instanceof ElectricalBrain electricalBrain){
            ...
            electricalBrain.getBrain().render(stack, vertexconsumer, packedLight, OverlayTexture.NO_OVERLAY,-1);
            stack.popPose();
        }
    }
    ...
    private void renderTail(PoseStack stack, Type type, int light, MultiBufferSource buffer, Vec3[] segments, LivingEntity parent, float partial) {
        ...
        for (int i = 0; i < segments.length; i++) {
            Vec3 currentPos = segments[i];
            renderConnectionTail(origin, currentPos,type,light, stack, buffer, i,partial,color,i == 1,i == segments.length-1,i==segments.length/2,i<segments.length/2);
            origin = currentPos;
        }
    }
    private void renderConnectionTail(Vec3 from, Vec3 to,Type parent,int light, PoseStack stack, MultiBufferSource buffer,int index, float partial
            ,int color,boolean first,boolean last,boolean middle,boolean belowMiddle) {
        if (from == null || to == null) return;
        Vec3 direction = to.subtract(from);
        float length = (float) direction.length();
        if (length < 0.0001f) return;
    ...
    public void renderBit(PoseStack stack,int light,VertexConsumer consumer){
        bolt.renderToBuffer(stack,consumer,light, OverlayTexture.NO_OVERLAY, -1);
    }
}
