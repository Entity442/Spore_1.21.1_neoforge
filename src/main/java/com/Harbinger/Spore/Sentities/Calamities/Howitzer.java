...
public class Howitzer extends Calamity implements TrueCalamity, RangedAttackMob {
    ...
    protected void defineSynchedData(SynchedEntityData.Builder builder) {
        ...
        builder.define(RIGHT_ARM, this.getMaxArmHp());
        builder.define(LEFT_ARM, this.getMaxArmHp());
        builder.define(ORES, 0);
        builder.define(NUKE, 0);
        builder.define(SELF_DETONATION, 0);
    }
        ...
        for (BlockPos blockpos : BlockPos.betweenClosed(Mth.floor(aabb.minX), Mth.floor(aabb.minY), Mth.floor(aabb.minZ), Mth.floor(aabb.maxX), Mth.floor(aabb.maxY), Mth.floor(aabb.maxZ))) {
            ...
            if (blockstate.is(Utilities.biomass)){
                ...
                if (blockstate.getDestroySpeed(level(), blockpos) < getDestroySpeed() && blockstate.getDestroySpeed(level(), blockpos) >= 0 && EventHooks.canEntityGrief(this.level(), this)) {
                    if (blockstate.is(TagKey.create(Registries.BLOCK,ResourceLocation.parse("c:ores")))){
    ...
    public void searchBlocks(){
        ...
        for(BlockPos blockpos : BlockPos.betweenClosed(Mth.floor(aabb.minX), Mth.floor(aabb.minY), Mth.floor(aabb.minZ), Mth.floor(aabb.maxX), Mth.floor(aabb.maxY), Mth.floor(aabb.maxZ))) {
            ...
            if (block.is(TagKey.create(Registries.BLOCK, ResourceLocation.parse("c:ores")))){
