...
public class Verfalldrachen extends Calamity implements TrueCalamity, RangedAttackMob {
    ...
    protected void defineSynchedData(SynchedEntityData.Builder builder) {
        ...
        builder.define(SONIC_DATA_ID,-1);
        builder.define(TAR_DATA_ID,-1);
        builder.define(RIGHT_WING_HP,wingsMaxHp);
        builder.define(LEFT_WING_HP,wingsMaxHp);
        builder.define(TAR_HEAD_HP,headsMaxHp);
        builder.define(SONIC_HEAD_HP,headsMaxHp);
        builder.define(ELECTRICAL_HP,headsMaxHp);
        builder.define(TAR_HEAD_SEGMENTS,TAR_HEAD_SEGMENT);
        builder.define(SONIC_HEAD_SEGMENTS,SONIC_HEAD_SEGMENT);
    ...
    public void tick() {
        ...
        if (this.tickCount % 40 == 0 && this.getHealth() >= this.getMaxHealth()){
            ...
            if (getTarHeadSegment() < TAR_HEAD_SEGMENT){
                this.playSound(getTarHeadSegment() < (TAR_HEAD_SEGMENT-2) ? Ssounds.VERFALL_NECK_REGROW.value() : Ssounds.VERFALL_HEAD_REGROW.value());
            ...
            if (getElectricalHeadSegment() < ELECTRICAL_SEGMENT){
                this.playSound(getElectricalHeadSegment() < (ELECTRICAL_SEGMENT-2) ? Ssounds.VERFALL_NECK_REGROW.value() : Ssounds.VERFALL_HEAD_REGROW.value());
            ...
            if (getSonicHeadSegment() < SONIC_HEAD_SEGMENT){
                this.playSound(getSonicHeadSegment() < (SONIC_HEAD_SEGMENT-2) ? Ssounds.VERFALL_NECK_REGROW.value() : Ssounds.VERFALL_HEAD_REGROW.value());
...
// ==================== INDIVIDUAL HEAD ATTACKS ====================
    ...
    private void performTarHeadAttack(LivingEntity target) {
        ...
        setTarTargetId(-1);
        this.playSound(Ssounds.VERFALL_TAR_ATTACK.value());
    }
...
// ==================== UTILITY METHODS ====================
    ...
    public static class DragonFlyByGoal extends Goal {
        ...
        public void start() {
            ...
            dragon.level().playSound(dragon,target.blockPosition(),Ssounds.VERFALL_SPECIAL_ATTACK.value(),SoundSource.MASTER,1f,1f);
