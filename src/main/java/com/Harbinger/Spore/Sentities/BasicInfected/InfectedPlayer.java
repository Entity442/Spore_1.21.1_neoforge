...
public class InfectedPlayer extends Infected implements RangedAttackMob , ArmedInfected, EvolvingInfected, VariantKeeper {
    ...
    public void Evolve(Infected livingEntity, List<? extends String> value,ScamperVariants variants) {
        if (livingEntity != null && value != null && livingEntity.level() instanceof ServerLevel){
            ...
            if (Math.random() < 0.9) {
                ...
                ResourceLocation randomElement1 =  ResourceLocation.parse(value.get(randomIndex));
                ...
                if (waveentity instanceof Infected infected){
                    ...
                    if (level instanceof ServerLevel serverLevel){
                        infected.finalizeSpawn(serverLevel,serverLevel.getCurrentDifficultyAt(infected.getOnPos()),MobSpawnType.CONVERSION,null);
                    }
                }
    ...
    public void setVariant(int i) {
        this.entityData.set(DATA_ID_TYPE_VARIANT,i > InfPlayerSkins.values().length || i < 0 ? 0 : i);
    }
