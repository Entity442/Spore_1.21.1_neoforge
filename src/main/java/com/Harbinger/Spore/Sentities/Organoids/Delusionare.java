...
public class Delusionare extends Organoid implements VariantKeeper {
    ...
    private static final List<ResourceKey<Enchantment>> protectionEnchants = List.of(Enchantments.PROTECTION,Enchantments.PROJECTILE_PROTECTION,Enchantments.BLAST_PROTECTION,Enchantments.FIRE_PROTECTION);
    private static final List<ResourceKey<Enchantment>> speed = List.of(Enchantments.SWIFT_SNEAK,Enchantments.FEATHER_FALLING,Enchantments.DEPTH_STRIDER);
        ...
        if (value == Spells.SUMMON_ILLUSION.getId()){
            for (int i = 0;i<this.random.nextInt(2,5);i++){
                ...
                if (level() instanceof ServerLevel serverLevel)
                    illusion.finalizeSpawn(serverLevel,serverLevel.getCurrentDifficultyAt(this.getOnPos()), MobSpawnType.MOB_SUMMONED,null);
                ...
                if (mob instanceof LivingEntity living  && living instanceof Infected){
                    if (living instanceof ArmedInfected){
                        for (EquipmentSlot slot : EquipmentSlot.values()){
                            ResourceKey<Enchantment> enchantment = protectionEnchants.get(random.nextInt(protectionEnchants.size()));
                            Holder<Enchantment> holder = Senchantments.getEnchantment(level(),enchantment);
                            ItemStack stack = living.getItemBySlot(slot);
                            if (stack.getItem() instanceof ArmorItem){
                                Senchantments.EnchantItem(level(),stack,enchantment,random.nextInt(holder.value().getMaxLevel()));
                            }
                        }
                    }else {
