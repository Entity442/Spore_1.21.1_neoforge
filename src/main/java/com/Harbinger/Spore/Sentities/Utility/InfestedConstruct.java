...
public class InfestedConstruct extends UtilityEntity implements RangedAttackMob, Enemy, ColdWeakness {
    ...
    protected void defineSynchedData(SynchedEntityData.Builder builder) {
        ...
        builder.define(DISPENSER,false);
        builder.define(METAL_RESERVE,0);
        builder.define(MACHINE_HEALTH,(float)(maXmachineHp * 1f));
    }
    ...
    public boolean addEffect(MobEffectInstance effectInstance, @Nullable Entity entity) {
        if (effectInstance.getEffect().value().isBeneficial() || effectInstance.getEffect() == Seffects.CORROSION){
    ...
    public Map<Item,Integer> getValues(){
        ...
        for (String string : SConfig.SERVER.cons_blocks.get()){
            ...
            Item stack = Utilities.tryToCreateStack(ResourceLocation.parse(strings[0])).getItem();
            if (value > 0){
    ...
    public void tick() {
        super.tick();
        if (tickCount % 40 == 0){
            if (EventHooks.canEntityGrief(this.level(), this) && horizontalCollision){
                griefBlocks();
            }
            ...
            if (hasEffect(Seffects.CORROSION)){
    ...
    public void performDispenserShot(LivingEntity entity) {
        ...
        if (Math.random() < 0.4f){abstractarrow.setRemainingFireTicks(160);}
