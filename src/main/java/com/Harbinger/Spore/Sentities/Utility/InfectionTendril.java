...
public class InfectionTendril extends UtilityEntity {
    ...
    protected void defineSynchedData(SynchedEntityData.Builder builder) {
        ...
        builder.define(MOUND_AGE,2);
        builder.define(LIFE, 4800);
        ...
        for(BlockPos blockpos : BlockPos.betweenClosed(Mth.floor(aabb.minX), Mth.floor(aabb.minY), Mth.floor(aabb.minZ), Mth.floor(aabb.maxX), Mth.floor(aabb.maxY), Mth.floor(aabb.maxZ))) {
            ...
            if (Math.random() < 0.02 && blockstate.isSolidRender(level,blockpos)
                ...
                for (String str : SConfig.DATAGEN.block_infection.get()){
                    ...
                    Block blockBase = Utilities.tryToCreateBlock(ResourceLocation.parse(string[0]));
                    Block blockBase2 = Utilities.tryToCreateBlock(ResourceLocation.parse(string[1]));
                    if (blockstate.getBlock().equals(blockBase)){
