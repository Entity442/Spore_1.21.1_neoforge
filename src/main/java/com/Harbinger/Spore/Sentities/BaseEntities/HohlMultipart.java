...
public class HohlMultipart extends LivingEntity implements TrueCalamity {
    ...
    public void tick() {
        ...
        if (tickCount > 1) {
            ...
            if (!level().isClientSide) {
                if (parent == null || parent.isRemoved()  || (parent instanceof Hohlfresser leviathan && !Objects.equals(leviathan.getChildId(), this.uuid)) || parent.distanceTo(this) > 10) {
    ...
    protected void onEffectAdded(MobEffectInstance instance, @Nullable Entity source) {
        ...
        if (!instance.getEffect().value().isBeneficial() && instance.getAmplifier() < 2){
            return;
        }
